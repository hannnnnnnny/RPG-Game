## 装备生成 —— 等价于 src/core/lootGenerator.ts
##
## 用法：注册为 Autoload "LootGenerator"
## 调用：LootGenerator.generate_loot("enemy", world_tier)

extends Node

const SLOTS := [
	Types.SLOT_MAIN_HAND, Types.SLOT_CHEST, Types.SLOT_HANDS,
	Types.SLOT_BOOTS, Types.SLOT_RING, Types.SLOT_TOTEM
]
...
func _random_id(prefix: String) -> String:
	# Godot 没有 toString(36)，用十六进制 + 时间戳避免碰撞
