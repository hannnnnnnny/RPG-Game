...
- 开始 UI 优化：调整首屏、空背包、任务和矿区目标文案，减少调试感，让界面更贴近暗黑克苏鲁 RPG。
- 重做 CSS 视觉层级：统一暗黑色板、增加纹理背景、优化 HUD、侧栏面板、装备卡、对话框、选择面板和交互状态。
- UI 验证通过：桌面和移动端 Playwright 截图正常；角色创建页、游戏 HUD、对话框、侧栏面板均显示正常，移动端无横向溢出。
- 根据反馈优化角色可读性：玩家、感染矮人和创建页头像都改为能看到脸的 2D 小人表现。
